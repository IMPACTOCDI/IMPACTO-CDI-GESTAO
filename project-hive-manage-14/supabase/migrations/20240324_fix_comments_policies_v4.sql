...
RETURNS TRIGGER AS $$
BEGIN
    RAISE NOTICE 'Tentativa de inserção de comentário:';
    RAISE NOTICE 'auth.uid(): %', auth.uid();
    RAISE NOTICE 'NEW.created_by: %', NEW.created_by;
    RAISE NOTICE 'NEW.task_id: %', NEW.task_id;
    RAISE NOTICE 'NEW.project_id: %', NEW.project_id;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Adicionar trigger para debug
