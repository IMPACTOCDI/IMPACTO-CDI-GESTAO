...
-- Função auxiliar para verificar se usuário tem acesso ao projeto
    ...
    USING (
        EXISTS (
            ...
            AND (
                projects.visibility = 'public'::project_visibility
                OR projects.created_by = auth.uid()
                OR is_project_member(project_members.project_id)
            )
        )
    );
...
CREATE POLICY "Enable all access for authorized users"
